...
for ni = n
    ...
    for m= freq
        ...
        Dt =(lambda + 1i*omega)^2*exp(lambda*(2*delta+L))-(lambda-1i*omega)^2*exp(-lambda*(2*delta+L));
        a= ((lambda + 1i*omega )^2*exp(2*lambda*delta) - (lambda-1i*omega)^2*exp(-2*lambda*delta))/Dt;
        b= -(lambda^2 -(1i*omega)^2)*(exp(lambda*L) - exp(-lambda*L))/Dt;
...
saveas(gcf,'Convergence_factor','epsc')
figure(2)
